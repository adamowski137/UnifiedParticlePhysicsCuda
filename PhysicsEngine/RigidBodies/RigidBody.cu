...
__global__ void initializePositionsKern(int nParticles, float* x, float* y, float* z, int dim, int nOffset, float xOffset, float yOffset, float zOffset)
{
	...
	x[index + nOffset] = xIndex * ((PARTICLERADIUS + 0.01f) * 2) + xOffset;
	y[index + nOffset] = yIndex * ((PARTICLERADIUS + 0.01f) * 2) + yOffset;
	z[index + nOffset] = zIndex * ((PARTICLERADIUS + 0.01f) * 2) + zOffset;
}

void RigidBody::addRigidBodySquare(float* x, float* y, float* z, float* invmass, int startIdx, int n, float xOffset, float yOffset, float zOffset)
{
